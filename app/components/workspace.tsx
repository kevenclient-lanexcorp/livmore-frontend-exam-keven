...
const Workspace: React.FC<Props> = ({
  ...
  const price = lodash.last(value.workspace_prices)

  return (
    <div className="flex gap-6 bg-white rounded-2xl cursor-pointer p-3" onClick={onClick}>
      <div className="h-[146px] w-[129px] bg-[#868e96] rounded-xl"></div>
      <div className="flex flex-col gap-2 grow pr-4">
        <div className="flex items-center justify-between">
          <span className="font-bold">{value.name}</span>
          <span className="text-[#32915a] text-sm bg-[#eefff5] rounded-lg p-1.5">
            Available
          </span>
        </div>
        ...
        <div className="flex grow">
          <span className="flex gap-1 items-center text-[#787979] text-sm h-fit">
        ...
        <div className="flex justify-between">
          ...
          <Button type="primary" size="middle"
            onClick={(event) => {
              ...
              context.setWorkspaceBookingDetails(value)
            }}>
            Book
          </Button>
        </div>
